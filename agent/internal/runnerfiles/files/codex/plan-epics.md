...
this repository. The optional topic or PRD pointer is **$1** (the text after
`/plan-epics`; if `$1` appears literally un-substituted, none was given — ask
the human what to plan). Nothing here is autonomous — every epic ships only
...
1. Read `$1` if given — a topic or PRD path. Read any referenced docs.
...
Ask questions ONE AT A TIME, prefer multiple choice, and converge on:
...
labels: agentmon:epic, plan-gate, agent:codex
