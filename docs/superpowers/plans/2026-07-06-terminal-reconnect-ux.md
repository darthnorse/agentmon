...
- **Known limitation (pre-existing, slightly wider now):** desktop tiles are keyed by `paneIdentity` but the panes store dedupes by the session-name-inclusive key — if a tmux restart recycles a pane id into a *differently named* session and the user opens that row while the dead tile is still open (the ended banner makes leaving it open likelier), a second tile with a duplicate React key mounts. Same-named recreation (the common case) hits the dedupe branch and reconnects via the kick. Sibling of the accepted "wrong session name if focused pane's row vanishes" limitation.
