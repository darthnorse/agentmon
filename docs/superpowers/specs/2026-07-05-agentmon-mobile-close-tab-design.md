...
- **Close the *entered* tab, then reload (KNOWN LIMITATION, accepted):** the route's mount always re-adds the URL-encoded entered pane to the open set (entering = open), and in-state switch/close never advances the URL. So if you close the tab you originally entered on, keep working on a neighbor, then reload the page, the closed entered pane is re-seeded and reappears as a tab. Low impact — reload is uncommon on the mobile PWA, and it is consistent with the entering=open rule. Not fixed because the only real fix — advancing the URL on switch/close — would make tab-switching *navigate*, fighting the keep-alive design (switching is deliberately in-state, no route change, and is unit-tested as such). Accepted for v1.
