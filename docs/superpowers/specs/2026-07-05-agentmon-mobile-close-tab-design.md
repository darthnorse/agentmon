...
**Warming:** the eager-warm loop warms **open-set members** (up to `MOBILE_POOL_CAP`), not all rows. The focused/entered pane always counts first (unchanged). Sessions not in the open set are never warmed — no wasted sockets. Because open-set entries carry the pane parts (`serverId`/`target`/`paneId`) directly, warming no longer needs to wait for the live session list to load (today's loop is gated on `rows.length`).
