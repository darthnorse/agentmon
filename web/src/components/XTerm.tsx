...
>(function XTerm({ onData, onResize, fontSize = 13, theme = TERMINAL_THEMES.dark }, ref) {
  ...
  React.useEffect(() => {
    ...
    // Deliberately NO WebGL renderer. Its addon loads asynchronously after
    // term.open(), and on a freshly-created session tile that is still laying out
    // it can initialize at the wrong canvas size and then buffer output WITHOUT
    // painting — input reaches the pane, but the screen only updates on a resize
    // or a close+reopen. xterm's default renderer has no such init race and is
    // more than fast enough for agent-session throughput.
